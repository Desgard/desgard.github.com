...
请求时的缓存指令包括：`no-cache`（用于指示请求或响应消息不能缓存）、`no-store`、`max-age`、`max-stale`、`min-fresh`、`nly-if-cached`;
...
表明客户端希望接受GIF图象格式的资源；Accept：text/html，表明客户端希望接受html文本。
...
4：GET/JUNK/1.0发送一个不正确规格的Http协议请求。Http指纹识别工具Httprint,它通过运用统计学原理,组合模糊的逻辑学技术,能很有效的确定Http服务器的类型.它可以被用来收集和分析不同Http服务器产生的签名。
